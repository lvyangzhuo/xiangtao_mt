...
<div class="good-detail sift-mg">
	<h3 class="h_h3">短信下载：<font color="red"></font> </h3>
...
try {
	...
	if(c.equals("1"))
	{
		...
		*/
		...
		if (count>0) {
			...
			//余额积分必须大于所需积分才能下载
			if (yejf>=c_jf) {
				out.println("<form method=post name=f1 id=f1 action='/3g/sendsms.jsp?c=2'><div align=left>"
						+"短信内容：<span style='color:#325e7f;'>优惠券@"+c_sms+"【惠多网】</span>"
						+"<input type=hidden name=c_text value='优惠券@"+c_sms+"【惠多网】'>"
						+"<input type=hidden name=jid value='"+jid+"'>"
						+"<input type=hidden name=sid value='"+sid+"'>"
						+"<br>输入手机号：<input type=text name=c_tel id=c_tel maxlength=11 class='smstext'><br>"
						+"<span style='color:#666666;'>（中国移动、联通、电信号码都可以接收）</span></div>"
						+"<br><div align=center style='width:300px;'><input type=submit name=send id=send value=开始接收  style='width:80px;height:25px;'></div><br></form>");
			}else
			{
				out.println("<div align=left>"
	...
	}else if(!jid.equals("")&&c.equals("2"))
	{
		...
		if(i>0)
		{
			out.println("<div align=center style='width:300px;'>此优惠劵已发送到您手机，手机接收会有延迟，请耐心等待下。5分钟内还没收到可重发<br><a href='showinfo.jsp?id="+jid+"'>返回</a></div>");
			return;
		}
		...
		/* 此处用的短信网接口,账号和密码也可以自己申请充值,如用其他平台,把以下代码删除即可*/
		// 创建StringBuffer对象用来操作字符串
		StringBuffer sb = new StringBuffer("http://http.c123.com/tx/?");
		
		
		/*
		// 向StringBuffer追加用户名   账号密码向原作者索取 qq 11166250
		sb.append("uid=");
		...
		*/
		String inputline = "100";
			...
			if(inputline.equals("100"))
			{
				...
				out.println("<div align=center style='width:300px;'>短信优惠券已发送，接收会有几秒钟延迟，祝您消费愉快！<br><a href='showinfo.jsp?id="+jid+"'>返回</a></div>");
			}else
			{
				/*
				out.print("短信发送失败，请您稍后再试！<br><br>"
						...
						*/
				out.println("<div align=center style='width:300px;'>短信发送失败,请返回重试<br><a href='showinfo.jsp?id="+jid+"'>返回</a></div>");
