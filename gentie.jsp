...
String lgs = "&nbsp; <span class=kuo>[</span>&nbsp;<a href=\"login.jsp?rturl=/3g/news_info.jsp?id="+jid+"\" >登录</a>&nbsp;<span class=kuo>]</span>";
if(userid.length()>3)lgs = " &nbsp; <span class=kuo>"+userid+"</span>&nbsp; <span class=kuo>[</span>&nbsp;<a href=\"/logout.jsp?rturl=/3g/news_info.jsp?id="+jid+"\" target=_top>退出</a>&nbsp;<span class=kuo>]</span>";
...
try {
	...
	//精华
	sql = "select c_dtype,c_fee,c_text,c_userid,c_appdate,c_retext,c_redate,c_adm,c_guest from t_dianping where c_enable=10 and c_jid="+Tools.isNumber(jid)+"";
	...
	for (int i = 0; i < s.size(); i++) {
		...
		show += "<div class=\"fl com-a\"><img src=\""+c_avatar+"\" width=45 height=45 /></div>"
			...
			+ "     "+c_dtype+"<span class=\"rec\"><!-- 推荐： --></span><span class=\"gra\">分享了你的经验<!-- "+c_fee+" --></span><br />"
			+ "     "+c_text+"<br />"
			...
			*/
			...
			//show += "<li class=\"mu_l\"> <span class=\"mu_lh\"><img src=\""+c_avatar+"\" width=45 height=45 /><br><a href=\"javascript:;\" class=\"blu\">"+c_userid+"</a></span> <span class=\"mu_lc\">"+c_text+"</span></li>";
	}
	shownum = pagenow - 1;
	...
	System.out.println("gentie.jsp error :" + e);
	e.printStackTrace();
} finally {
...
<div class="parting-line"></div>
	 <div id="pag">&nbsp;<a href="news_info.jsp?npage=<%=shownum%>&id=<%=jid%>&jid=<%=jid%>&f=<%=f%>&j=<%=j%>" class="last" >上一页</a> <a href="news_info.jsp?npage=<%=fy%>&id=<%=jid%>&jid=<%=jid%>&f=<%=f%>&j=<%=j%>" class="next"  >下一页</a> 
	 &nbsp;&nbsp; <span class="red"><%=pagenow%></span>/<%=pagenum%> 页 <span class="red"><%=count%></span> 个跟帖</div>
...
<ul class="mu_lw">
			...
			<li class="mu_l"><span class="mu_lh"></span> <span class="mu_lc q-txt"><input name="c_fee" id="c_fee" type="hidden" class="wbktext" value="share" /> <span class="blu"></span></span></li>
