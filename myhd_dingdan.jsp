...
if(c_userid.equals("")||c_userid.length()<1){
	response.sendRedirect("/3g/login.jsp?rturl=/3g/myhd_dingdan.jsp");
	return;
}
...
try {
	...
	sql ="select rid,titles,enable,money,appdate,name,mobile,address,dtime,peisong,email,zip,tel from t_dingdan where appdate>='"
	...
	for (int i = 0; i < s.size(); i++) {
		...
		dtime = (String)obj[8];
		peisong = (String)obj[9];
		email = (String)obj[10];
		zip = (String)obj[11];
		sidlist = (String)obj[12];
		...
		//查订单详情
		list = "";
		sql = "select t_dingass.rid,t_news.titles,t_shops.c_sname,t_dingass.money,t_dingass.num from t_dingass,t_news,t_shops where t_dingass.did="+c_id+" and t_dingass.rid=t_news.rid and t_news.ctype_b=t_shops.c_id";
		rs = stmt.executeQuery(sql);
		while(rs.next())
		{
			list += "<tr>   <td>"+rs.getString(2)+"</td>    <td>"+rs.getString(3)+"</td>    <td>"+rs.getString(4)+".0元</td>    <td align=center>"+rs.getString(5)+" </td>  </tr>  ";
		}
		rs.close();
		...
		switch(Tools.isNumber(c_act)){
			case 0:
				c_act = "<font color=#fd9929>已下单</font>";
				if (peisong.indexOf("在线支付")>-1){c_act = "<font color=#fd9929>未支付</font>";
				zhifu = "<form name=alipayment"+i+" action=\"/3g/zhifubao/alipayapi2.jsp\" method=post target=\"_blank\"><input name=\"WIDseller_email\" type=\"hidden\"  value=\"cy@ding58.cn\"/><input name=\"WIDout_trade_no\" type=\"hidden\"  value=\""+c_id+"\"/><input name=\"WIDsubject\" type=\"hidden\"  value=\""+c_titles+"\"/><input name=\"WIDtotal_fee\" type=\"hidden\"  value=\""+c_pname+".0\"/><input name=\"WIDbody\" type=\"hidden\"  value=\""+name+"\"/><input name=\"WIDshow_url\" type=\"hidden\"  value=\"http://www.ding58.cn/waimai/shop_"+shopid+".html\"/><input type=\"Submit\" name=\"Submit\" value=\"付款\"  /></form>";}
				break;
			case 1:
				c_act = "<font color=#fd9929>已下单</font>";
				if (peisong.indexOf("在线支付")>-1){c_act = "<font color=#fd9929>已下订单未支付</font>";
				zhifu = "<form name=alipayment"+i+" action=\"/3g/zhifubao/alipayapi2.jsp\" method=post target=\"_blank\"><input name=\"WIDseller_email\" type=\"hidden\"  value=\"cy@ding58.cn\"/><input name=\"WIDout_trade_no\" type=\"hidden\"  value=\""+c_id+"\"/><input name=\"WIDsubject\" type=\"hidden\"  value=\""+c_titles+"\"/><input name=\"WIDtotal_fee\" type=\"hidden\"  value=\""+c_pname+".0\"/><input name=\"WIDbody\" type=\"hidden\"  value=\""+name+"\"/><input name=\"WIDshow_url\" type=\"hidden\"  value=\"http://www.ding58.cn/waimai/shop_"+shopid+".html\"/><input type=\"Submit\" name=\"Submit\" value=\"付款\"  /></form>";}
				break;
			case 2:
			...
			case 3:
				...
				break;
			case 4:
				c_act = "<font color=green>已领取</font>";
				zhifu = "";
				break;
			case 5:
				c_act = "<font color=green>完成</font>";
				zhifu = "";
				break;
			case 6:
				c_act = "<font color=#fd9929>故未完成</font>";
				zhifu = "";
				break;
			case 7:
				c_act = "<font color=green>已付款</font>";
				zhifu = "";
				break;
			case 9:
		...
		list = "<table width='100%' border='0' style='font-size:12px'>  <tr align='center' style='font-weight:bold'>    <td>商品</td>	<td>所属商家</td>    <td>单价</td>    <td align=center>数量</td>  </tr>"+list+sidlist+"<tr>    <td></td>        <td align=right>总价：</td>    <td>"+c_pname+".0元</td><td></td>    </tr>  <tr><td colspan=4>订购人："+name+"</td></tr><tr><td colspan=4>联系电话："+mobile+"</td></tr><tr><td colspan=4>送货地址："+address+"</td></tr><tr><td colspan=4>送货时间："+dtime+"</td>     </tr><tr> <td colspan=4>订单号："+did+"&nbsp;状态：&nbsp;"+c_act+"</td></tr><tr><td  colspan=4>短信验证码：<B>"+email+"</B>  （消费时请出示）</td><tr></table>";

		
		str += "<tr style='font-size:12px'>"
			+ "    <td height=\"70\" class=\"bgtd tit2\" align=center><input type=\"checkbox\" name=\"cid\" value=\""+c_id+"\" /></td>"
			+ "    <td align=\"center\" class=\"bgtd\">"+c_act+zhifu+"</td>"
			+ "    <td align=\"center\" class=\"bgtd\"><a href=\"javascript:;\" onClick=\"sText('txt"+c_id+"')\" >"+c_titles+"<br>【查看详情】</a></td>"
			+ "    <td align=\"center\" class=\"bgtd\">"+c_pname+".0元</td>"
			+ "    <td align=\"center\" class=\"bgtd2\">"+c_appdate+"<span id='txt"+c_id+"' style=\"display:none\">"+list+"</span></td>"
			+ "  </tr>";

	}
	if(s.size()==0)str = "<tr>"
			+ "    <td height=\"70\" class=\"bgtd tit2\" colspan=3>　　您还没有使用过外卖订餐，<a href='shop_wm.jsp' target=_blank>快来看看吧</a></td>"
	...
	*/
	shownum = pagenow - 1;
   	...
   	System.out.println("/3g/myhd_dingdan.jsp error Exception :" + e);
}finally
{
	if (stmt != null) {
		stmt.close();
...
xinxi = "共有 <em>"+count+"</em> 条，当前第 <em>"+pagenow+"</em> 页<!-- ，共 <em>"+pagenum+"</em> 页 -->";
...
if(links.indexOf("javascript")<0)links = "myhd_dingdan.jsp?npage="+ shownum + "&act=" + act + "&stime="+stime+"&etime="+etime+"&c="+c+"";
if(linkn.indexOf("javascript")<0)linkn = "myhd_dingdan.jsp?npage="+ pagenow + "&act=" + act + "&stime="+stime+"&etime="+etime+"&c="+c+"";
if(link1.indexOf("javascript")<0)link1 = "myhd_dingdan.jsp?npage=1&act=" + act + "&stime="+stime+"&etime="+etime+"&c="+c+"";
if(link9.indexOf("javascript")<0)link9 = "myhd_dingdan.jsp?npage="+ pagenum + "&act=" + act + "&stime="+stime+"&etime="+etime+"&c="+c+"";
%>
...
function sText(id)
{
	dialog("查看订单详细内容","id:"+id+"","299px","300px","window");
}
</script>
...
<div class="good-detail sift-mg">
  ...
  <div class="titles">
  	<ul>
    	<li ><strong class="dabt">我的订单</strong></li>
        <li class="more"></li>
    </ul>
  </div>
  ...
  <tr class="biaoge_top" style='font-size:12px'>
    <td width="15%" height="37" class="bgtm"></td>
    <td width="15%" class="bgtm">状态</td>
    <td width="40%" class="bgtm" align=center>名称</td>
...
<tr>
	...
	<input type="checkbox" name="all" id="checkedAll"  /> 全选</td>
  	<td colspan="4" class="bgtd" >
	   ...
	   &nbsp;
	    <input type="button" value="设为已领取" name="sjh" id="sjh">
       &nbsp;
	    <input type="button" value="设为未领取" name="wsh" id="wsh">
        &nbsp; 
  ...
  <tr>
	<td class="bgtd tit2" align=center> &nbsp;</td>
  	<td colspan="4" class="bgtd" ><span id=note></span></td>
...
<script language="javascript">
	...
	$("#wsh").click( function () {
		...
		if(i>0){
			dialog("选为用户未领取","url:/comp/dd_update.jsp?c=3"+cid,"280px","130px","window");
	...
	$("#sjh").click( function () {
		...
		if(i>0){
			dialog("选为已领取","url:/comp/dd_update.jsp?c=4"+cid,"280px","130px","window");
			setTimeout(shuaxin,2000);
			return true;
			}
		else {
