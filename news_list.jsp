...
<%@ page language="java" import="java.util.regex.Pattern"%><%
	...
	String c_sid = Tools.RmFilter(request.getParameter("sid")); //商家id号
	if (mm.equals("2")) {
	...
	if (!c_sid.equals("")) {
		whereSql = "and t_shops.c_id="+Tools.isNumber(c_sid)+"";
	}
	...
	*/
	...
	sql = "select c_id,c_sname,c_address,c_tel,c_logo,c_jnum,c_vip,c_map,c_hao,c_dps,t_news.rid,t_news.titles,t_news.price from t_shops,t_news where c_vip=2 and (c_enable<9 or c_enable=11)"
		+ whereSql + " and t_shops.c_id=t_news.ctype_b and (t_news.ctype='cp' or t_news.ctype='tg' or t_news.ctype='wm')"+whereSql+" order by rid desc,c_id desc";
	...
	try {
		...
		if(rs.next())
		{
			...
			{
				c_remark += "<a href=\"news_list.jsp?name=[name]&c=[c]&j=[j]&a="+java.net.URLEncoder.encode(keys[jj], "GBK")+"\"  class=searchbtn2>"+keys[jj]+"</a> ";
			}
			...
			for(int jj=0;jj<keys.length&&jj<3;jj++)
			{
				c_range += "<a href=\"news_list.jsp?name=[name]&c=[c]&q="+java.net.URLEncoder.encode(keys[jj], "GBK")+"&j=[j]\"  class=searchbtn2>"+keys[jj]+"</a> ";
			}
		}
		rs.close();
	...
	for (int d = fn; d < en && d <= pagenum; d++) {
		...
		if (d == pagenow) {
			...
			morepage += "<a href=\"news_list.jsp?npage=" + d + "&name="
					+ name + "&c=" + c + "&g="+g+"&q=" + q + "&j="+j+"&a=" + a + "&sid="+c_sid+"\" >"
					+ d + "</a>";
		}
	...
	strb = Tools.replaceAll(strb,"[shang]","<a href=\"news_list.jsp?npage="
			+ shownum + "&name=" + name + "&c=" + c + "&g="+g+"&q=" + q + "&j="+j+"&a="
			+ a + "&sid="+c_sid+"\" class=\"last\">&nbsp;</a>");
	
	strb = Tools.replaceAll(strb, "[上一页]", "<a href=\"news_list.jsp?npage="
			+ shownum + "&name=" + name + "&c=" + c + "&g="+g+"&q=" + q + "&j="+j+"&a="
			+ a + "&sid="+c_sid+"\" class=\"last\">上一页</a>");
	strb = Tools.replaceAll(strb, "[下一页]", "<a href=\"news_list.jsp?npage="
			+ pagenow + "&name=" + name + "&c=" + c + "&g="+g+"&q=" + q + "&j="+j+"&a="
			+ a + "&sid="+c_sid+"\" class=\"next\">下一页</a>");
