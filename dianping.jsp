...
if(j.equals("1"))whereSql = " c_enable=2"; 
...
String lgs = "&nbsp; <span class=kuo>[</span>&nbsp;<a href=\"login.jsp?rturl=/3g/showshop.jsp?id="+sid+"\" >登录</a>&nbsp;<span class=kuo>]</span>";
if(userid.length()>3)lgs = " &nbsp; <span class=kuo>"+userid+"</span>&nbsp; <span class=kuo>[</span>&nbsp;<a href=\"/logout.jsp?rturl=/3g/showshop.jsp?id="+sid+"\" target=_top>退出</a>&nbsp;<span class=kuo>]</span>";
...
try {
	...
	//精华
	sql = "select c_dtype,c_fee,c_text,c_userid,c_appdate,c_retext,c_redate,c_adm,c_guest from t_dianping where "+whereSql+" and c_sid="+Tools.isNumber(sid)+" ";	//and c_jid=0
	...
	for (int i = 0; i < s.size(); i++) {
		...
		c_fee = Tools.RmNull((String)obj[1]);
		...
		show += "<div class=\"fl com-a\"><img src=\""+c_avatar+"\" width=45 height=45 /></div>"
			...
			+ "     "+c_text+"<br />"
			+ "     <span class=\"rep\"><a href=\"javascript:;\" onClick=\"jubao('举报:点评_"+((pagenow-1)*10+i+1)+"_shop_"+sid+".html')\">举报</a></span><span class=\"dat1\">"+c_appdate+"</span></div>"
			+ "   "+c_retext+""
			+ "   <div class=\"spa\"></div>";
			*/
	...
	if(s.size()==0){
		show = "<div class=\"parting-line\"></div><a href=\"javascript:;\" class=\"good-lnk2\" style=\"margin-left:10px\">暂无评论～</a>";
		if(j.equals("1"))show = "<div class=\"fl \"></div><div class=\"com-n fr\" style='width:650px'><a href='###' onClick='setFocus()'>暂无精华点评，快来点评吧～</a></div><div class=\"spa\"></div>";
	}
	...
	System.out.println("dianping.jsp error :" + e);
	e.printStackTrace();
} finally {
...
<ul class="mu_lw">
					<a href="javascript:;" class="good-lnk2" style="margin-left:10px"> <B>网友评论</B> </a>	
						<%=show%>
		</ul>

<div class="parting-line"></div>
	 <div id="pag">&nbsp;<a href="showshop.jsp?npage=<%=shownum%>&id=<%=sid%>&sid=<%=sid%>&f=<%=f%>&j=<%=j%>" class="last" >上一页</a> <a href="showshop.jsp?npage=<%=fy%>&id=<%=sid%>&sid=<%=sid%>&f=<%=f%>&j=<%=j%>" class="next"  >下一页</a> 
...
<ul class="mu_lw">
			...
			<li class="mu_l"><span class="mu_lh"><span class="nm" onClick="xzhong()">匿名</span> <input type="checkbox" name="niming" id="niming" /></span> <span class="mu_lc q-txt">&nbsp;&nbsp;<input type="image" id="tidp"  name="imageField" src="/images/tj.gif" /></span></li>
