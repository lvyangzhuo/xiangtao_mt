...
DBcon db=new DBcon();
	...
	//取区域，和商圈信息
		...
		if(rs.next())
		{
			...
			{
				c_remark += "<span><a href=\"shop_list.jsp?a="+keys[jj]+"\" class=searchbtn2>"+keys[jj]+"</a></span>";
				//c_remark += "<span><input type=button name=quyu"+jj+" value='"+keys[jj]+"' class=searchbtn2 onClick=\"window.location.href='shop_list.jsp?a="+keys[jj]+"'\" ></span>";

				//list += "<span><input type=button name=quyu"+jj+" value='"+keys[jj]+"' class=searchbtn2 onClick=\"window.location.href='shop_list.jsp?a="+keys[jj]+"'\" > </span>";
			}
			...
			for(int jj=0;jj<keys.length;jj++)
			{
				c_range += "<span><a href=\"shop_list.jsp?q="+keys[jj]+"\"  class=searchbtn2>"+keys[jj]+"</a></span>";
				//c_range += "<span><input type=button name=quyu"+jj+" value='"+keys[jj]+"' class=searchbtn2 onClick=\"window.location.href='shop_list.jsp?q="+keys[jj]+"'\" ></span>";
			}
		}
		rs.close();
		c_remark = c_remark+"<span><a href=\"shop_list.jsp\" class=searchbtn2>全部区域</a></span>";

		c_range = c_range+"<span><a href=\"shop_list.jsp\" class=searchbtn2>全部商圈</a></span>";
